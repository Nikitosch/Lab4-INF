...
def double_teg(row,spaces):
    teg_name = search("<\w*",row)[0]
    row_between_tegs= search("(?<=>).*(?=<)",row)[0] # row[row.find(">")+1:row.rfind("<")].strip()
    row_with_ravno = search("(?<=\s).*(?=>)",row)[0] # row[row.find(" ")+1:row.find(">")]
    return convert_sms(row_with_ravno, spaces, teg_name, row_between_tegs)

def solo_teg(row,spaces):
    teg_name = search("(?<=<)\w*(?=\s)",row)[0] # row[row.find("<")+1:row.find(" ")]
    row = search("(?<=\s).*(?=/)",row)[0] # row[row.find(" ")+1:row.find("/")].strip()
    return convert_sms(row,spaces,teg_name)

def double_no_last_teg(row,spaces):
    teg_name = search("(?<=<)\w*(?=>)",row)[0] #row[row.find("<") + 1:row.find(">")]
    row_between_tegs = search("(?<=>).*",row)[0] #row[row.find(">") + 1:].strip()
    row_with_ravno = search("(?<=\s).*(?=>)",row)[0] #row[row.find(" ") + 1:row.find(">")] if " " in row else ""
    print(row_between_tegs, row_with_ravno)
    return convert_sms(row_with_ravno, spaces, teg_name, row_between_tegs)

def solo_no_last_teg(row,spaces):
    teg_name = search("(?<=<)\w*(?=\s)",row)[0] #row[row.find("<") + 1:row.find(" ")]
    row = search("(?<=\s).*",row)[0] #row[row.find(" ") + 1:].strip()
    return convert_sms(row,spaces,teg_name)
...
def parse_row(row:str, result, spaces):
    if row.startswith("<?xml") or not row or ("</" in row and row.count(">") == 1):
        ...
        if row.startswith("<") and not row.startswith("</"):
            ...
            if result == []:
                row = row.strip()
                row = " "*4 +row[row.find(" ")+1:-1].replace("=", ":") #search("(?<=\s)\w*",row)[0][::-1].replace("=",":")
                result.append(row)
            elif row.count(">") == 2:
